...
# Audit-Fix #19: fail-loud bei fehlenden Tokens.
# =============================================================================
set -uo pipefail
...
# Dedup (5min) außer bei confirm
if [ "$NO_RATE_LIMIT" -ne 1 ] && [ "$SEVERITY" != "confirm" ]; then
  HASH=$(echo "$SEVERITY|$SUBJECT" | md5sum 2>/dev/null | cut -d' ' -f1)
  HASH=${HASH:-$(echo "$SEVERITY|$SUBJECT" | md5 2>/dev/null)}  # macOS fallback
...
if [ -x "$SCRIPT_DIR/notify-slack.sh" ]; then
  if bash "$SCRIPT_DIR/notify-slack.sh" "$CHANNEL" "$SUBJECT" "$BODY" "$SEVERITY" 2>/dev/null; then
    SLACK_OK=1
  fi
fi

# Pushover (für blocker/confirm/warning)
PUSHOVER_OK=0
...
# WhatsApp (nur blocker/confirm)
WA_OK=0
if [ "$WHATSAPP" -eq 1 ] && [ -x "$SCRIPT_DIR/notify-whatsapp.sh" ]; then
  if bash "$SCRIPT_DIR/notify-whatsapp.sh" "$SUBJECT" "$BODY" 2>/dev/null; then
    WA_OK=1
  fi
fi

# Audit-Fix #19: fail-loud bei kritischen ohne jeglichen Channel
