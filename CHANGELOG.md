...
### Fixed
- **9 bestätigte Funde aus dem adversarialen Verify-Pass vor Release** (3 Linsen, 14 Agenten, jeder Fund mit Sandbox-Repro durch einen Skeptiker bestätigt; 2 weitere Behauptungen wurden als anforderungslose Hardening-Nits verworfen). Statusline: der Codepoint-Filter ließ den C1-Control-Block U+0080–U+009F durch — die 8-Bit-Formen von CSI/OSC/DCS/NEL, also live ANSI-Injection auf C1-fähigen Terminals trotz „Injection ist tot"-Zusage. Update-Hook, vier Wege auf denen Teil B still und dauerhaft gestorben wäre bzw. gegen den Opt-out gefeuert hätte: `export WORK_CONVENTION_AUTO_UPDATE=off` (und führende Leerzeichen) umgingen das sed-Gate; ohne existierendes `~/.claude` scheiterte der Lock-`mkdir` am fehlenden Parent (das reparierende `mkdir -p` stand dahinter — toter Code); ein Zukunfts-Timestamp im Throttle-Marker vergiftete den Throttle für immer (wird jetzt als fehlend behandelt); ein verwaister Lock mit Inhalt (z.B. `.DS_Store`) war per `rmdir` unlöschbar (jetzt `rm -rf`). Teil A zählte leere Verzeichnisse und verirrte Dateien im Cache als „bereitliegende Version" und hätte eine dauerhaft falsche Neustart-Zeile produziert (zählt jetzt nur Versions-Dirs mit `plugin.json`). Doku-Leichen: TROUBLESHOOTING erwartete noch „Registered 7 hooks", SETUP nannte „11 Hook-Scripts". Und das STATUSLINE.md-Snippet warf ohne installierte ≥2.1.0 bei jedem Render `bash ""`-Fehler (jetzt `[ -n ]`-Guard). Alle 9 als Regressionstests verewigt; Suite 66 → 103.
